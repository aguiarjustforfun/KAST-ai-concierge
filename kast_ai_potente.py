...
'pl': {
    ...
    intent = get_intent(query)
    name = "Tomás"
    balance = 1250.75  # fictício – depois podes ligar a BD real
    time_now = datetime.now().strftime("%d/%m/%Y %H:%M")

    response = f"Olá {name}! 👋 Saldo atual: {balance:.2f} USDC. "

    if intent == 'depósito':
        response += "Para depositar: App → Wallet → Deposit (USDC, SOL, etc.). Se tens tx hash, envia aqui!"
    elif intent == 'saldo':
        response += "O teu saldo é 1250.75 USDC. Queres ver últimas transações?"
    elif intent == 'cartão':
        response += "Cartão KAST: ativo em 160+ países, sem taxas forex. Status: Ativo."
    elif intent == 'fees':
        response += "Fees: 0% em swaps internos, ~1% em saques fiat, zero em viagens."
    elif intent == 'viagens':
        response += "Ideal para viagens! Cartão aceito globalmente + Apple Pay/Google Pay."
    elif intent == 'suporte':
        response += "Precisas de suporte humano? Abre ticket no app ou email support@kast.xyz"
    elif intent == 'yield':
        response += "Yield: até 4.5% APY em USDC (em breve). Ativa em App → Grow."
    elif intent == 'cashback':
        response += "Cashback: até 5–8% dependendo do tier. Pontos atuais: 420."
    else:
        response += "Não entendi bem a pergunta. Tenta reformular (ex: 'saldo', 'depositar', 'cartão', 'taxas')"
...
if __name__ == '__main__':
    print("Servidor Flask a correr em http://127.0.0.1:5000")
